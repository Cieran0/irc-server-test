...
void client::handle() {
    while(m_info.username.empty() || m_info.real_name.empty()) {
        std::string message = get_next_message();
        if(message.starts_with("NICK ")) {
            m_info.nickname= message.substr(5);
            server_controller.add_to_client_map(m_info.nickname, this);
            std::cout << "Username " << m_info.nickname << std::endl;
        } else if (message.starts_with("USER ")) {
            m_info.username = split_string(message," ", 0)[1];
            int name_start = message.find_first_of(':');
            m_info.real_name = message.substr(name_start+1);
            std::cout << "Real name " << m_info.real_name << std::endl;
    ...
    server_controller.client_map.emplace(m_info.nickname, this);

    //TODO figure this out
    send_message(":"+server_controller.host_name+" 001 "+m_info.nickname+" :Hi, welcome to IRC\r\n");
    send_message(":"+server_controller.host_name+" 002 "+m_info.nickname+" :Your host is "+server_controller.host_name+", running version miniircd-2.3\r\n");
    send_message(":"+server_controller.host_name+" 003 "+m_info.nickname+" :This server was created sometime\r\n");
    send_message(":"+server_controller.host_name+" 004 "+m_info.nickname+" "+server_controller.host_name+" miniircd-2.3 o o\r\n");
    send_message(":"+server_controller.host_name+" 251 "+m_info.nickname+" :There are 2 users and 0 services on 1 server\r\n");
    send_message(":"+server_controller.host_name+" 422 "+m_info.nickname+" :MOTD File is missing\r\n");
    ...
    while (true)
    {
        ...
        if(message.starts_with("JOIN ")) {
            ...
            server_controller.get_channel(channel_name).join(m_info.nickname);
            ...
            send_message(":"+server_controller.host_name+" 331 "+m_info.nickname+ " " + channel_name + " :No topic is set\r\n");
            ...
            send_message(":"+server_controller.host_name+" 353 "+m_info.nickname + " = " + channel_name+ " :" + user_raw + "\r\n");
            send_message(":"+server_controller.host_name+" 366 "+m_info.nickname+ " " + channel_name+ " :End of NAMES list\r\n");

            for(std::string nickname : user_list) {
                if(nickname != m_info.nickname)
                    server_controller.send_message_to_client(nickname, join_message);
            }
            ...
            for (std::string user : current_channel.get_users()){
                std::cout << "USER: " << user << std::endl;
                clients_info.push_back(server_controller.get_client_info(user));
            }
...
std::string generate_who_response(const std::string& requesting_nick, const std::vector<client_info>& clients, const std::string& channel) {
    ...
    for (const auto& client : clients) {
        response << ":" << server_controller.host_name << " 352 " << requesting_nick << " " 
                 << channel << " " << client.username << " " 
                 << client.ip << " " << server_controller.host_name << " " 
                 << client.nickname << " H"  // Always 'H' for "Here"
                 << " :0 " << client.real_name << "\r\n";
    }
    
    // Always send the End of WHO list response (315), even if the list is empty
    response << ":" << server_controller.host_name << " 315 " << requesting_nick << " " 
